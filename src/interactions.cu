...
__host__ __device__ float GeometrySchlickGGX(float NdotV, float roughness) {
    ...
    float k = (roughness * roughness) / 8.0f; // 注意: 直接光照通常用 k=(r+1)^2/8, IBL用 k=r^2/2。这里统一用直接光照的近似
...
// 辅助：计算高光采样的概率权重 (用于 Sample 和 PDF 保持一致)
__host__ __device__ float CalculateSpecularProbability(const Material& m, const glm::vec3& N, const glm::vec3& V) {
    ...
    return glm::clamp(specProb, 0.05f, 0.95f);
}
...
__host__ __device__ float pdfBSDF(const glm::vec3& wo, const glm::vec3& wi, const glm::vec3& N, const Material& m) {
    ...
    // 2. Specular PDF (GGX VNDF -> Jacobian -> Solid Angle)
    float D = DistributionGGX(N, H, roughness);
    // pdf_h = D * (N dot H)
    // pdf_omega = pdf_h / (4 * (wo dot h))
    float NdotH = glm::max(glm::dot(N, H), 0.0f);
    float pdfSpec = (D * NdotH) / (4.0f * VdotH + 0.0001f);
    ...
    thrust::default_random_engine& rng)
{
    ...
    if (r_select < specProb) {
        // --- Specular Sampling (NDF Importance Sampling) ---
        // 1. 生成半程向量 H
        float a = roughness * roughness;
        ...
        // --- Diffuse Sampling (Cosine Hemisphere) ---
    ...
    pdf = pdfBSDF(wo, wi, N, m);
    return evalBSDF(wo, wi, N, m);
}
    ...
    float& pdf)
{
    ...
    // 2. 【关键修正】缩小到 0.5 以匹配 Intersection Logic
